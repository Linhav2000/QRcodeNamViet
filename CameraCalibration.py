...
class CameraCalibration:
    ...
    def undistort(self, image):
        ...
        # undistort
        dst = cv.undistort(image, self.mtx, self.dist, None, newcameramtx)
        return dst
...
if __name__ == '__main__':
    ...
    for fname in images:
        ...
        caliImage = cal.undistort(adjustedImage)
        print(f"time == {TimeControl.time() - time}")
        cv.imencode(".jpg", caliImage)[1].tofile(f"./CaliImage/cali_{i}.jpg")
        # cv.imwrite(f"./CaliImage/cali_{i}.jpg", caliImage)

        caliImage = cv.resize(caliImage, (800, 600))
        cv.imshow("imag", caliImage)
        cv.waitKey(1000)
        i += 1
